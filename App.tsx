...
import { GardenBed, Vegetable, VeggieType, SunOrientation, BedLayout, BedShape, SeedVariety } from './types';
...
const App: React.FC = () => {
  const [beds, setBeds] = useState<GardenBed[]>([
    { id: '1', name: 'Main Bed', width: 48, height: 96, x: 5, y: 5, shape: 'rectangle' },
  ]);
  ...
  useEffect(() => {
    const gardenKeys = Object.keys(localStorage).filter(k => k.startsWith('garden_v1_'));
    setSavedGardens(gardenKeys.map(k => k.replace('garden_v1_', '')));
    const plantingKeys = Object.keys(localStorage).filter(k => k.startsWith('planting_v1_'));
  ...
  const handleSavePlanting = (name: string) => {
    ...
    if (!savedPlantings.includes(name)) setSavedPlantings([...savedPlantings, name]);
  };
  ...
  const handleUpdateSeed = (type: VeggieType, priority: number) => {
    setSeeds(prev => prev.map(s => s.type === type ? { ...s, priority } : s));
  };

  const handleUpdateVarieties = (type: VeggieType, varieties: SeedVariety[]) => {
    setSeeds(prev => prev.map(s => s.type === type ? { ...s, selectedVarieties: varieties } : s));
  };
  ...
  const handleGenerate = async () => {
    ...
    if (activeVegetables.length === 0) {
      alert("Please increase the priority of at least one vegetable to generate a layout.");
      return;
    }

    setIsGenerating(true);
    try {
      const result = await generateGardenLayout(beds, activeVegetables, sunOrientation);
      setLayouts(result);
    } catch (err) {
      console.error(err);
  ...
  const onMouseMove = useCallback((e: MouseEvent) => {
    ...
    const rawX = (e.clientX - rect.left - pan.x) / (GRID_SIZE * zoom) - dragOffset.current.x;
    const rawY = (e.clientY - rect.top - pan.y) / (GRID_SIZE * zoom) - dragOffset.current.y;

    const x = Math.max(0, Math.round(rawX));
    const y = Math.max(0, Math.round(rawY));

    setBeds(prev => prev.map(b => b.id === draggedBedId ? { ...b, x, y } : b));
      ...
      <main className="flex-1 relative flex flex-col overflow-hidden">
          ...
          <div className="flex gap-8 items-center">
            <div className="flex items-center gap-3">
               ...
               <div className="flex flex-col">
                 <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest leading-none mb-1">Canvas Mode</span>
                 <span className="text-xs font-bold text-slate-900">Precision Planning</span>
               </div>
            </div>
            ...
            <div className="flex items-center gap-3">
               ...
               <div className="flex flex-col">
                 <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest leading-none mb-1">Solar Bias</span>
                 <span className="text-xs font-bold text-slate-900">{sunOrientation} Axis</span>
               </div>
            </div>
          </div>
          ...
          {selectedBed && (
            <div className="flex gap-6 animate-in fade-in slide-in-from-right-4 duration-500 items-center bg-white px-5 py-2.5 rounded-2xl border border-slate-400 shadow-2xl">
              <div className="flex flex-col gap-1.5 pr-6 border-r border-slate-300">
                <span className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Name</span>
                <input 
                  type="text"
                  ...
                  onChange={(e) => {
                    ...
                    setBeds(prev => prev.map(b => b.id === selectedBedId ? { ...b, name: val } : b));
                  }}
              ...
              <div className="flex flex-col gap-1.5 pr-6 border-r border-slate-300">
                <span className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Shape</span>
                <select 
                  ...
                  onChange={(e) => {
                    ...
                    setBeds(prev => prev.map(b => b.id === selectedBedId ? { 
                      ...b, 
                      shape: newShape,
                      height: newShape === 'circle' ? b.width : b.height
                    } : b));
                  }}
                >
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Size (inches)</span>
                  ...
                  {selectedBed.shape !== 'circle' && (
                    <>
                      ...
                      <input 
                        ...
                        onChange={(e) => setBeds(prev => prev.map(b => b.id === selectedBedId ? { ...b, height: parseInt(e.target.value) || 0 } : b))}
                      />
                    </>
                  )}
                </div>
        ...
        >
          ...
          <div className="absolute top-8 right-8 flex flex-col items-center opacity-80 pointer-events-none z-30">
            <div className="text-[12px] font-black mb-1 text-slate-900 tracking-tighter">N</div>
            <div className="w-16 h-16 border-2 border-slate-400 rounded-full flex items-center justify-center relative bg-white backdrop-blur-sm shadow-xl">
               ...
               <div className={`absolute w-2.5 h-12 bg-gradient-to-t from-orange-500 to-orange-700 rounded-full transition-transform duration-1000 ease-in-out origin-bottom shadow-md ${
                 sunOrientation === 'North' ? 'rotate-0' : 
                 sunOrientation === 'East' ? 'rotate-90' : 
                 sunOrientation === 'South' ? 'rotate-180' : 'rotate-270'
               }`} style={{ top: -24 }}></div>
               <div className="w-4 h-4 rounded-full bg-slate-500 border-2 border-white shadow-inner z-10"></div>
            </div>
            <div className="text-[12px] font-black mt-1 text-slate-900 tracking-tighter">S</div>
          </div>
        </div>
          ...
          {Object.entries(VeggieType).map(([key, value]) => {
            ...
            return (
              <div key={key} className="flex items-center gap-2 group cursor-default">
                <div className="w-4 h-4 rounded-full border border-black/20 shadow-sm transition-transform group-hover:scale-125" style={{ backgroundColor: meta.color }}></div>
                <span className="text-[11px] font-black text-slate-900 group-hover:text-emerald-800 transition-colors uppercase tracking-[0.15em]">{value}</span>
              </div>
            );
          })}
        </div>

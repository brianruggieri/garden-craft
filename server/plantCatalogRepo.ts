...
import { PLANT_CATALOG } from "../shared/plantCatalog";
import type { PlantCatalog } from "../shared/types";

async function loadCatalogFromJsonEnv(): Promise<PlantCatalog | null> {
  const raw = process.env.PLANT_CATALOG_JSON;
  if (!raw) return null;
  try {
    return JSON.parse(raw) as PlantCatalog;
  } catch (err) {
...
async function loadCatalogFromFile(): Promise<PlantCatalog | null> {
...
async function loadCatalogFromDatabase(): Promise<PlantCatalog | null> {
  if (!process.env.PLANT_CATALOG_DB_URL) return null;
  console.warn("PLANT_CATALOG_DB_URL configured but DB client not wired.");
  return null;
}

export async function getPlantCatalog(): Promise<PlantCatalog> {

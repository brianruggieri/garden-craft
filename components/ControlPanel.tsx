...
import { GardenBed, Vegetable, VeggieType, SunOrientation, SeedVariety, BedLayout } from '../types';
...
const ControlPanel: React.FC<ControlPanelProps> = ({
  ...
  const [editingVarieties, setEditingVarieties] = useState<VeggieType | null>(null);
  ...
  const toggleVariety = (vType: VeggieType, variety: SeedVariety) => {
    ...
    if (exists) {
      onUpdateVarieties(vType, current.filter(v => v.id !== variety.id));
    } else {
    ...
    <div className="w-80 h-full bg-white border-r border-slate-200 flex flex-col shadow-lg overflow-y-auto z-10">
      <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1 uppercase tracking-tighter font-bold">Varietal Optimizer</p>
        </div>
        <button 
        ...
        {showVault && (
          <section className="animate-in fade-in slide-in-from-top-4 duration-300">
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Saved Garden Frameworks</span>
                  <button onClick={handlePromptSaveGarden} className="text-[10px] text-indigo-600 font-bold hover:underline">Save Current</button>
                </div>
                <div className="max-h-32 overflow-y-auto space-y-1">
                  {savedGardens.length === 0 && <p className="text-[10px] text-slate-400 italic">No gardens saved.</p>}
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Saved Planting Plans</span>
                  <button onClick={handlePromptSavePlanting} className="text-[10px] text-indigo-600 font-bold hover:underline">Save Current</button>
                </div>
                <div className="max-h-32 overflow-y-auto space-y-1">
                  {savedPlantings.length === 0 && <p className="text-[10px] text-slate-400 italic">No plans saved.</p>}
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 flex justify-center">
               <button onClick={() => setShowVault(false)} className="text-[10px] font-bold text-slate-400 uppercase hover:text-slate-600">Close Vault</button>
            </div>
          </section>
        )}
          ...
          <>
            <section>
              ...
              <div className="grid grid-cols-4 gap-1">
                {(["North", "South", "East", "West"] as SunOrientation[]).map(dir => (
                  <button
                    key={dir}
                ...
                {beds.map(bed => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <button 
                        onClick={(e) => { e.stopPropagation(); onRemoveBed(bed.id); }} 
                    ...
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-tighter mt-1">
                      {bed.shape === 'circle' ? `Diameter: ${bed.width}"` : `${bed.width}" width × ${bed.height}" height`}
                    </span>
                  </div>
                ))}
              </div>
            ...
            <section>
              <h2 className="font-bold text-slate-800 text-sm mb-4 flex items-center gap-2">
                <i className="fas fa-leaf text-green-500"></i> Varieties & Priority
              </h2>
              <div className="space-y-6">
                {Object.values(VeggieType).map(vType => {
                  ...
                  return (
                    <div key={vType} className="bg-slate-50 rounded-lg p-3 border border-slate-100 transition-all hover:border-slate-300">
                      <div className="flex justify-between items-center mb-2">
                        <div className="flex items-center gap-2">
                          <span className="text-lg">{VEGGIE_METADATA[vType].icon}</span>
                          <span className="text-xs font-bold text-slate-700">{vType}</span>
                        </div>
                        <button 
                          onClick={() => setEditingVarieties(isEditing ? null : vType)}
                          className={`text-[10px] px-2 py-0.5 rounded-full font-bold border transition-colors ${
                            selectedCount > 0 ? 'bg-green-100 text-green-700 border-green-200' : 'bg-slate-100 text-slate-400 border-slate-200'
                          }`}
                        >
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        <input
                          ...
                          step="1"
                          value={priority}
                          onChange={(e) => onUpdateSeed(vType, parseInt(e.target.value))}
                          className="flex-1 h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-green-600"
                        />
                        <span className="text-[10px] font-black text-slate-400 w-4">{priority}</span>
                      </div>

                      {isEditing && (
                        <div className="mt-3 pt-3 border-t border-slate-200 space-y-2 animate-in slide-in-from-top-2 duration-200">
                          <p className="text-[9px] font-bold text-slate-400 uppercase mb-1">Select Actual Varieties</p>
                          {SEED_VARIETY_LIBRARY.filter(v => v.type === vType).map(variety => {
                            const isSelected = seed?.selectedVarieties.some(sv => sv.id === variety.id);
                            return (
                              <button
                                key={variety.id}
                              ...
                              >
                                <div className="flex justify-between items-center">
                                  <span className="text-[10px] font-bold text-slate-700">{variety.name}</span>
                                  {isSelected && <i className="fas fa-check text-green-500 text-[8px]"></i>}
                                </div>
                                <p className="text-[8px] text-slate-400 leading-tight mt-0.5">{variety.description}</p>
                                <div className="flex gap-1 mt-1">
                                  <span className="text-[7px] bg-slate-100 px-1 rounded text-slate-500">Space: {variety.spacing}"</span>
                                  <span className="text-[7px] bg-slate-100 px-1 rounded text-slate-500">Root: {variety.rootDepth}</span>
                                </div>
                              </button>
                            );
                          })}
          ...
          >
            {isGenerating ? (
              ...
              <>
                ...
                <span className="text-[9px] opacity-60 text-white/70">Considers Spacing & Vertical Niches</span>
              </>
            )}
          </button>
        </div>
